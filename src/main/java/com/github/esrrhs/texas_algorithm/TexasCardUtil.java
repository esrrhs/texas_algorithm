...
public class TexasCardUtil
{
	...
	public static int fiveFromSix(ArrayList<Poke> handCards, ArrayList<Poke> publicCards, ArrayList<Poke> pickedCards)
	{
		if (publicCards.size() < 4)
		{
			return 0;
		}
		if (handCards.size() < 2)
		...
		pickedCards.clear();
		ArrayList<Poke> tmpSixCards = new ArrayList<>();
		for (Poke poke : handCards)
		{
			tmpSixCards.add(poke);
		}
		for (int i = 0; i < 4; ++i)
		{ //只取前4张
			tmpSixCards.add(publicCards.get(i));
		}
		//对七张牌排序
